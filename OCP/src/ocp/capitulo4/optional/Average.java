...
public class Average {
	...
	public static void main(String[] args) {
		...
		// System.out.println(x1.orElseThrow(() -> new ArithmeticException())); // throw
		// exception if empty
		System.out.println("Map-------------------- Get the amount of digits of a Integer");
		Optional<Integer> opInt = Optional.of(500);
		opInt.map(xxx -> ("" + xxx).length()).ifPresent(System.out::println);
		
		opInt.map(xxx -> ("" + xxx).length()).ifPresent(System.out::println);
		//public Optional<Integer> calculateSize()
	}
}
